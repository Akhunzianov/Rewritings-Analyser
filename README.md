...
###### Достоинства подхода:
• Быстрая скорость работы _O(n<sup>2</sup>m<sup>2</sup>)_ где _n_ – количество текстов в датасете, _m_ – cреднее количество слов длины не менее 3 в одном тексте. 
...
## Подход 5: TF-IDF (Term Frequency - Inverse Document Frequency)
Данный алгоритм реализован Ахунзяновым Ренатом. В реализации используются файлы `sample.json`, `TF-IDF.py`.

Результат работы данного метода записан в файле `rewritings_tf-idf.json`.
...
1. Считывание из файла выполняется аналогично с другими подходами
2. Рассмотрим функцию `dictionary_add_tf`:
   ```def dictionary_add_tf(data_dictionary):
    length_data = len(data_dictionary)
    for i in range(1, length_data + 1):
        data_dictionary[i] = [data_dictionary.get(i, 0), tf_computation(data_dictionary[i])]
   ```
   Эта функция добавляет в словарь `data_dictionary` список TF для слов данного текста, которое считается в функции `tf_computation`.

3. Рассмотрим функцию `tf_computation`:
   ```def tf_computation(words):
    ...
    return tf
   ```
Для каждого слова заданного текста она вычисляет отношение количества повторений этого слова в тексте и общего числа слов в тексте, что и является TF. Возвращает она словарь с ключами - словами текста и значениями - TF слов.

4. Рассмотрим функцию `idf_computation`:
   ```def idf_computation(corpus):
    ...
    return idf
   ```
   Данная функция проходит по всем текстам и каждое слово добавляет в словарь, в котором считает количество текстов, в которые входит слово, а затем вычисляет логарифм отношения общего числа документов и значения словаря для каждого слова. Благодаря этому алгоритм присваивает большие значения более редким словам в датасете, что даёт возможность выше ценить повотрение редких слов в текстах.

5. Рассмотрим функцию `find_tfidf_rewritings`:
   ```def find_tfidf_rewritings(data_dictionary, idf_dictionary):
    ...
    return rewriting_dictionary
   ```
   Данная функция для каждого предложения вычисляет TF-IDF для каждого слова, что являеся произведением TF и IDF очередного слова. Затем мы сравниваем кадые тексты попарно, вычисляя евклидово рассотяние между векторам TF-IDF. В нашем случае это значит, что мы проверяем, что в предложениях хотя бы 75% слов совпадает и тогда считаем корень из суммы квадратов разностей TF-IDF слов, которые встречаются в обоих текстах. И если данная велчина оказалась не больше 1, считаем тексты ререайтиногом и записываем их в словарь рерайтингов.

6. Запись в файл происходит аналогично осталным подходам

###### Достоинства подхода:
• Достаточно высокая скорость работы алгоритма. Он выполняется за
время _O(n<sup>2</sup>m<sup>2</sup>+mn)_, где _n_ – количество текстов в датасете, _m_ – cреднее
количество слов длины не менее 3 в одном тексте
• Достаточно высокая эффективность выявления рерайтингов, в которых
используются перестановка слов, удаление слов, использование
параллельных конструкций
•   Благодаря умножению на IDF алгоритм хорошо выявляет ререайтинги, в
которых используются редкие слова для данного датасета
• Хорошо преминем для сравнения больших текстов

###### Недостатки подхода:
• Алгоритм плохо выявляет рерайтинги, в которых форма многих слов
изменена и в которых используются синонимы, из-за этого результаты его работы иногда могут быть не совсем точными
